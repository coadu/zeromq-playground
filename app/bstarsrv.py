...
def connect(sock, address):
    while True:
        try:
            ...
            # TODO: raise pull request for more detail error with pyzmq
            # pyzmq throws generic error, the underlying problem (we are trying to catch) is that the name is not resolved
            time.sleep(1)
            pass
        except:
            raise
...
def main():
    ...
    if args.primary:
        ...
        connect(statesub, "tcp://backup.pyzmq.dev.docker:5003")
        fsm.state = STATE_PRIMARY
    elif args.backup:
        print("I: Backup slave, waiting for primary (master)")
        frontend.bind("tcp://*:5000")
        statepub.bind("tcp://*:5003")
        connect(statesub, "tcp://primary.pyzmq.dev.docker:5003")
        statesub.setsockopt_string(zmq.SUBSCRIBE, u"")
